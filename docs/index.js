...
function isPalindrome(str) {
  // Tu solución acá
  const stringInvertido = reverseString(str);
  return str === stringInvertido;
}
...
function closestPair(arr) {
  ...
  // Inicializar la variable para almacenar el par con la mínima diferencia
  let parMinDiferencia = [arr[0], arr[1]];
  let minDiferencia = Math.abs(parMinDiferencia[1] - parMinDiferencia[0]);

  // Recorrer el array desde el segundo elemento
  for (let i = 1; i < arr.length; i++) {
    const diferenciaActual = Math.abs(arr[i] - arr[i - 1]);

    // Si la diferencia actual es menor que la mínima diferencia, actualizar el par con la mínima diferencia
    if (diferenciaActual < minDiferencia) {
      minDiferencia = diferenciaActual;
      parMinDiferencia = [arr[i - 1], arr[i]];
    }
  }

  return parMinDiferencia;
}
